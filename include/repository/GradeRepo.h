...
namespace repo {
    class GradeRepo {
        ...
        AVLTree<model::HashTableKey> key_tree_;
    ...
    public:
        ...
        [[nodiscard]] model::HashTableKey * keys(size_t& count) const;
    ...
    inline GradeRepo::GradeRepo(const std::string &file_path, const ToKey to_key) {
        ...
        // дерево ключей
        key_tree_ = AVLTree < model::HashTableKey > ();
        ...
        for (std::size_t i = 0; i < grades_.size(); ++i) {
            const auto key = to_key_(grades_[i].get_student_name(), grades_[i].get_student_birth_date());
    ...
    inline bool GradeRepo::add_grade(model::Grade& grade) {
        ...
        const auto ids = key_tree_.to_arr(to_key_(grade.get_student_name(),
                grade.get_student_birth_date()), ids_cnt, []{});
        if (ids != nullptr) {
    ...
    inline bool GradeRepo::del_grade(const model::Grade &grade) {
        ...
        if (idx != grades_.size() - 1) {
            ...
            key_tree_.replace(to_key_(
                ...
                last_grade.get_student_birth_date()
                ),
                grades_.size() - 1, idx
            );
        }
    ...
    GradeRepo::search_grades(const model::HashTableKey &key, size_t &steps) const {
    ...
    inline model::HashTableKey * GradeRepo::keys(size_t &count) const {
