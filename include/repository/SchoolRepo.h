...
namespace repo {
    ...
    public:
        ...
        const model::Student *search_student(const model::HashTableKey &key, size_t &steps);

        bool add_grade(model::Grade &grade);
        bool del_grade(const model::Grade &grade);
        size_t del_grades(const model::HashTableKey &key);
        Vector<model::Grade> search_grades(const model::HashTableKey &key, size_t &steps) const;
    ...
    inline bool SchoolRepo::del_student(const model::Student &student) {
        ...
        const model::HashTableKey key = to_key_(student.get_name(), student.get_birth_date());
    ...
    inline const model::Student *SchoolRepo::search_student(const model::HashTableKey &key, size_t &steps) {
    ...
    inline bool SchoolRepo::add_grade(model::Grade &grade) {
        ...
        auto *student = student_repo_.search_student(
            to_key_(grade.get_student_name(), grade.get_student_birth_date()), tmp
        );
        if (student == nullptr) {
    ...
    inline Vector<model::Grade> SchoolRepo::search_grades(const model::HashTableKey &key, size_t &steps) const {
    ...
    inline Vector<model::StudentGrade> SchoolRepo::get_filtered(
        ...
        for (const auto & gr : grades) {
            ...
            model::HashTableKey key = to_key_(gr.get_student_name(), gr.get_student_birth_date());
