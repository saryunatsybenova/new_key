...
namespace app {
    class App {
        static model::HashTableKey to_key(const model::PersonName &pn, const model::Date &birth_date);
    ...
    inline model::HashTableKey App::to_key(const model::PersonName &pn, const model::Date &birth_date) {
        if (birth_date.to_string().empty()) {
            throw std::invalid_argument("Дата рождения не указана");
        }
        return model::HashTableKey(pn, birth_date);
    }
